...
contract CrowdfundingTest is Test {
    ...
    function test_Create_RaisedStartsAtZero() public {
        _create();
        (,,,,uint256 raised,,) = cf.getCampaign(1);
        assertEq(raised, 0);
    }
    ...
    function test_Create_ClaimedStartsFalse() public {
        _create();
        (,,,,,bool claimed,) = cf.getCampaign(1);
        assertFalse(claimed);
    }
    ...
    function test_Contribute_GoalReachedExactly() public {
        _create();
        vm.prank(bob);
        cf.contribute{value: GOAL}(1);
        (,,,,uint256 raised,,) = cf.getCampaign(1);
    ...
    function test_Extend_ContributeAfterExtension_Succeeds() public {
        _create();
        vm.prank(alice);
        cf.extendCampaign(1, 7 days);
        skip(DURATION + 1); // past original deadline
        vm.prank(bob);
        cf.contribute{value: CONTRIB}(1); // should succeed in extended window
        assertEq(cf.getContribution(1, bob), CONTRIB);
    }


    function test_Extend_ClaimAfterExtension_Succeeds() public {
        _create();
        vm.prank(alice);
        cf.extendCampaign(1, 7 days);
        vm.prank(bob);
        cf.contribute{value: GOAL}(1);
        skip(DURATION + 7 days + 1);
        vm.prank(alice);
        cf.claimFunds(1);
        (,,,,,bool claimed,) = cf.getCampaign(1);
        assertTrue(claimed);
    ...
    function test_SetReferralRate_EmitsEvent() public {
        vm.expectEmit(false, false, false, true);
        emit ReferralRateUpdated(100, 300);
    ...
    function test_Unpause_AllowsCreateCampaign() public {
        cf.pause();
        cf.unpause();
        vm.prank(alice);
        uint256 id = cf.createCampaign("title", "desc", GOAL, DURATION);
    ...
    function test_Integration_FullCycle_Success() public {
        // Create → Contribute → Claim
        vm.prank(alice);
        uint256 id = cf.createCampaign("Full Cycle", "desc", GOAL, DURATION);
        vm.prank(bob);
        cf.contribute{value: GOAL}(id);
        skip(DURATION + 1);
        uint256 aliceBefore = alice.balance;
        vm.prank(alice);
        cf.claimFunds(id);
    ...
    function test_Integration_FullCycle_Refund() public {
        // Create → Contribute → Fail → Refund
        vm.prank(alice);
        uint256 id = cf.createCampaign("Fail Cycle", "desc", GOAL, DURATION);
        vm.prank(bob);
        cf.contribute{value: 0.1 ether}(id);
        skip(DURATION + 1);
        uint256 bobBefore = bob.balance;
        vm.prank(bob);
        cf.refund(id);
